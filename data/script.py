...
class MAINdealabs:
    ...
    """
    def __init__(self, url, range, await_time, keyword, webhook, platform):
        self.url = url
        self.range = range
    ...
    def monitor(self):
        """
        func monitoring dealabs website
        """

        f = -1 #found / -1 first product not count
        x = 0 #range
    ...
    def webhook_sender(self, title, dealabs_link, site_link, img, price, old_price, thread_id, voucher_code, merchant):
        """
        send webhook dealabs monitor
        """
        # calc discount %
        try :
            if old_price != 'none' :
                origin = old_price.replace('€', '').strip()
                remise = price.replace('€', '').strip()

                result = int(float(remise)) * 100 / int(float(origin)) - 100
                discount_price = f'{result:.0f}%'
            else :
                discount_price = 'none'
        except :
            discount_price = 'none'
        ...
        color = 864410 #classic webhook color blue
        found = ''
        try :
            for items in self.keyword :
                    if items.lower() in title.lower():
                        color = 55813 #change color to green
        ...
        try :
            ...
            embed = {
                ...
                "fields": [
                    ...
                    {"name": "**__PRICE / OLD PRICE / %__**", "value": f'{price} / {old_price} / {discount_price}', "inline": False},
                ],
